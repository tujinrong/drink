...
(function () {
    ...
    smat.dynamics.Control.prototype = {
        ...
        getUiConfig: function (options) {
            ...
            if (this.config.designing) uiConfig.designing = this.config.designing;
            ...
            if (options != undefined && options != null) {
                ...
                if (typeof (options) == "function") {
                    ...
                    if (smat.dynamics.logicHandler) {
                        options.uiName = this.config.name;
        ...
        unActive: function () {
            ...
            if (!this.config.page.settingData) {
                this.config.page.propertysPanel.refreshControlList();
                this.config.page.active();
            }
        }, optionIgnoreInfo: {
            "parent": 1,
            "page": 1,
            ...
            if (this.config.parent != undefined
                ...
                && (this.config.parent instanceof smat.dynamics.mobile.Page == false)
            ...
            return c;
        }, getControlByTypes: function (typeArray, controls) {
            var cs = controls;
            if (!cs) cs = [];

            if (!typeArray) return cs;

            if (this.children != undefined) {
                for (var ckey in this.children.data) {
                    var ctl = this.children.data[ckey];

                    if ($.Enumerable.From(typeArray).Any("$ == '" + ctl.config.type + "'")) {
                        cs.push(ctl);
                    }

                    ctl.getControlByTypes(typeArray, cs)
                }
            }
        ...
        , setFormData: function (controls, beforeAddChild) {
            ...
            for (var key in controls) {
                ...
                if (this.config.page && this.config.page.config.print) {
                    ctl.cConfig.print = this.config.page.config.print;
                }
            }

            controls = $.Enumerable.From(controls)
                 .OrderBy("$.colIndex").ThenBy("$.Seq")
                .ToArray();
            ...
            for (var key in controls) {
                ...
                child.setFormData(ctl.Controls, beforeAddChild)
        ...
        propertyChange_htmlAttribute: function (property, value) {
            ...
            if (this.config.authName && this.config.page.authSet && !this.config.page.authSet[this.config.authName]) {
                return false;
            }
            return true;
        }
