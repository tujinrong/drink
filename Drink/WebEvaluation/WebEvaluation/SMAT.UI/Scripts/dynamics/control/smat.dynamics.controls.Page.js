...
(function () {
    ...
    smat.dynamics.Page.prototype = {
        ...
        init: function () {
            ...
            if (this.config.print == true) designClass += " s-dy-print ";
            this.body = $('<section id="page_' + this.uuid + '" class="scrollable wrapper s-dy-page ' + designClass + " " + this.getClassStr() + '"></section>').appendTo(this.config.contextOn);
        ...
        addChild: function (config) {
            ...
            if (smat.dynamics[config.type] != undefined) {
                ...
                if (!this.settingData && this.propertysPanel) {
            ...
            while (names[result.toLowerCase()] != undefined) {
                index++;
                result = name + "" + index;
            }
            ...
            names[this.config.name.toLowerCase()] = 1;
            ...
            //page auth
            if (smat.security.menuAuths) {
                if (smat.security.menuAuths["/" + this.config.entityName + "/" + this.config.name]) {
                    this.authSet = smat.security.menuAuths["/" + this.config.entityName + "/" + this.config.name];
                } else if (this.config.parentPageId) {
                    var parentPage = this.pagerSender._getParentPageControl();
                    if (parentPage && parentPage.authSet) {
                        this.authSet = parentPage.authSet;
                    }
                }
            }
            ...
            if (this.config.designing == true) {
                this.propertysPanel.refreshControlList();
                this.active();
            } else if (!this.config.noLoadEvent) {
                smat.service.uiAfterInit(this.body);
                if (this instanceof smat.dynamics.PageUserControl) {
                    if (!this.cached) {
                        this.trigger(this.config.loaded, { sender: this });
                    }
                } else {
                    this.triggerChildrenLoaded();
                    this.trigger(this.config.loaded, { sender: this });
                }
        ...
        }, triggerChildrenLoaded: function () {

            var userControls = this.body.find(".s-userControl");
            $.each(userControls, function () {
                var ucUi = $(this).ui();
                if (ucUi && ucUi.page.cached) {
                    ucUi.page.trigger(ucUi.page.config.loaded, { sender: ucUi.page });
                }
            ...
            if (this.config.groupName) {
                ...
                smat.service.loadJosnData({
                    ...
                    async: false,
                    success: function (result) {
                        self.groupForms = result.ds["Y_EntityForm"];

                        if (self.groupForms.length > 0) {
                            self.groupForms = $.Enumerable.From(self.groupForms).OrderBy("Number($.GroupSeq)").ToArray();

                            if (self.config.groupType == "2") {
                                var formFilter = "";
                                for (var index in self.groupForms) {
                                    var lForm = self.groupForms[index];
                                    formFilter += "'" + lForm.FormName + "',"
                                ...
                                params.request.DsRequests.push(
                                   {
                                       TableName: "Y_EntityFormControl",
                                       Filter: "ProjID = '" + self.config.projID + "' and FormName in(" + formFilter.substr(0, formFilter.length - 1) + ") and ControlType = 'Form'"
                                   }
                                );
                                ...
                                smat.service.loadJosnData({
                                    ...
                                    async: false,
                                    success: function (result) {
                                        var formDic = $.Enumerable.From(result.ds["Y_EntityFormControl"]).ToDictionary("$.FormName");
                                        self.linkViews = [];
                                        for (var index in self.groupForms) {
                                            var lForm = self.groupForms[index];
                                            var v = formDic.Get(lForm.FormName)
                                            if (v) {
                                                var formOptions = smat.service.strToJson(v.ControlOptions);
                                                self.linkViews.push({
                                                    ViewName: formOptions.view,
                                                    EntityName: lForm.EntityName,
                                                    FormName: lForm.FormName,
                                                    LinkGroupTitle: lForm.GroupName,
                                                    LinkGroupName: lForm.GroupLinkDesc
                                                });
                                            }
                                        }
                                    }
            ...
            if (smat.dynamics.analysisConfig && smat.dynamics.analysisConfig.linkToForm) {
                smat.dynamics.analysisConfig.linkToForm(this);
            } else {
            ...
            if ($("#" + fillTarget).length > 0) {
                ...
                smat.service.openForm({
                    ...
                    afterOpen: function () {
                        ...
                        if (self instanceof smat.dynamics.PageUserControl) {
                            if (!self.cached) {
                                prePage.trigger(prePage.config.loaded, { sender: prePage });
                            }
                        } else {
                            prePage.triggerChildrenLoaded();
                            prePage.trigger(prePage.config.loaded, { sender: prePage });
                        }
