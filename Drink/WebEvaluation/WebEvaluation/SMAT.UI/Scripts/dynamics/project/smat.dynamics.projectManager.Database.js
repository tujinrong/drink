(function () {
    ...
    smat.dynamics.ProjectManagerDatabase.prototype = {
        ...
        init: function () {
            ...
            //datas = $.Enumerable.From(datas).Where("$.EntityName.indexOf('Y_') < 0").ToArray();

            var treeDs = [];


            var groups = $.Enumerable.From(datas).GroupBy("$.EntityGroup").ToArray();
            ...
            for (var i = 0; i < datas.length; i++) {
                ...
                var gname = datas[i].EntityGroup;
                if (!gname) gname = "other";

                var g = $.Enumerable.From(treeDs).Where("$.text == '" + gname + "'").ToArray()[0];
