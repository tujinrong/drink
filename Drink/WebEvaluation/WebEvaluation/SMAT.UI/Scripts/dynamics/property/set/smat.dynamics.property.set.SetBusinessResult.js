    ...
    smat.dynamics.property.set.SetBusinessResult.prototype = {
        initDom: function () {
                ...
                var editColumns = [
                            ...
                            template: function (dataItem) {
                                ...
                                if (dataItem.itemFormat) {
                                    ...
                                    if (dataItem.itemFormat.indexOf("=Name(") == 0) {
                                        ...
                                        return smat.service.optionSet("CodeType." + optKey) + "名称表示";
                        ...
                        },
                        {
                            ...
                            template: function (dataItem) {
                                ...
                                var result = "";
                                if (hasUp) {
                                    result = result+'<button class="btn-primary s-button item-up" seq="' + dataItem.seq + '"  style="padding:2px;min-width: 16px;" >↑</button>';
                                } else {
                                    result = result + '<button class="s-button s-state-disabled" style="padding:2px;min-width: 16px;" disabled="disabled">↑</button>';
                                }

                                if (hasDown) {
                                    result = result + '<button class="btn-primary s-button item-down"  seq="' + dataItem.seq + '" style="padding:2px;min-width: 16px;" >↓</button>';
                                } else {
                                    result = result + '<button class=" s-button s-state-disabled" style="padding:2px;min-width: 16px;" disabled="disabled">↓</button>';
                                }
            ...
            params.request.DsRequests.push(
               {
                   ...
                   Filter: "ProjID = " + this.view.ProjID + " and AnalysisEntityName = '" + viewItem.EntityName + "' and AnalysisKeyType = '" + viewItem.ItemCategory + "' and EntityName = '" + viewItem.ItemEntityName + "' and FieldName = '" + viewItem.ItemFieldName + "'"
               }
            );
            ...
            smat.service.loadJosnData({
                ...
                async: false,
                success: function (result) {
                    if (result.ds["Y_AnalysisField"].length > 0) {
                        self.analysisFieldItem = result.ds["Y_AnalysisField"][0];
                    }
                    smat.service.closeLoding();
            ...
            if (viewItem.ItemCategory == "2") {
                if (self.analysisFieldItem) {

                    if (self.analysisFieldItem.IsSum) {
                        groupItem.push({ text: "集計", value: "Sum" });
                    }
                    if (self.analysisFieldItem.IsCount) {
                        groupItem.push({ text: "件数", value: "Count" });
                    }
                    if (self.analysisFieldItem.IsAvg) {
                        groupItem.push({ text: "平均", value: "Avg" });
                    }
                    if (self.analysisFieldItem.IsMax) {
                        groupItem.push({ text: "最大", value: "Max" });
                    }
                    if (self.analysisFieldItem.IsMin) {
                        groupItem.push({ text: "最小", value: "Min" });
                    }
            ...
            //        type: "Button",
            //        click: function (e) {
            //            var wid = $(e.sender.element).closest(".panel.panel-default").attr("id");
            ...
            if (self.analysisFieldItem && self.analysisFieldItem.DataType == "Date") {
                formatItem = [
            ...
            } else if (self.analysisFieldItem && self.analysisFieldItem.IsSum) {
                formatItem = [
                    { text: "カンマ", value: "{0:N0}" }
                ];
            } else if (self.analysisFieldItem && self.analysisFieldItem.EntityName == "T_Period") {
                formatItem = [

                ];
            } else if (!self.analysisFieldItem && viewItem.ItemCategory == "2") {
                formatItem = [
                    { text: "カンマ", value: "{0:N0}" }
                ];
            ...
            if (filterItem) {
                if (fieldItem) {
                    ...
                    filterText = filterItem.FilterSql.replace(self.analysisFieldItem.VirtualSql + " ", "");
            ...
            if (havingFilterItem && viewItem.Group) {
                if (fieldItem) {
                    ...
                    if (self.analysisFieldItem.VirtualSql == "*") {
                        havingFilterText = havingFilterItem.FilterSql.replace(" COUNT(*) ", "");
                    } else {
                        havingFilterText = havingFilterItem.FilterSql.replace("DISTINCT " + " ", "");
                        havingFilterText = havingFilterItem.FilterSql.replace(viewItem.Group + "(" + self.analysisFieldItem.VirtualSql + ")" + " ", "");
                    }
                    ...
                    if (result) {
                        ...
                        if (result.FilterText) {
                            ...
                            if (fieldItem) {
                                ...
                                filterItem.FilterSql = self.analysisFieldItem.VirtualSql + " " + result.FilterText;
                            } 
                        ...
                        if (result.HavingFilterText) {
                            ...
                            if (fieldItem) {
                                ...
                                } else if (viewItem.Group == "Count") {
                                    havingFilterItem.FilterSql = viewItem.Group + "(DISTINCT " + self.analysisFieldItem.VirtualSql + ")" + " " + result.HavingFilterText;
                                } else {
                                    havingFilterItem.FilterSql = viewItem.Group + "(" + self.analysisFieldItem.VirtualSql + ")" + " " + result.HavingFilterText;
                                }
                            }
                ...
                checkResult: function (result,uid) {
                    if (result.FilterText) {
                        ...
                        if (fieldItem) {
                            ...
                            if (self.analysisFieldItem) {
                                fs = self.analysisFieldItem.VirtualSql+ " " + fs;
                                en = self.analysisFieldItem.EntityName;
                            } else {
                                fs = " 0 " + fs;
                ...
                if (this.resultUi.config.resultType == "SetSummaryCross") {
                    ...
                    if (xViewItem.length > 0) {
                        ...
                        if (xViewItem[0].ItemName.indexOf("Date") > 0 || xViewItem[0].ItemName.indexOf("date") > 0) {
                            ...
                            if (this.resultUi.config.category_format == "=Date(YM)") {
                                this.resultUi.config.category_format = "YM";
                            } else if (this.resultUi.config.category_format == "=Date(Y)") {
                                this.resultUi.config.category_format = "Y";
                            }  else {
                ...
                if (this.resultUi.config.resultType == "SetPieChart") {
                    ...
                    if (sViewItem.length > 0) {
                        ...
                        if (sViewItem[0].ItemName.indexOf("Date") > 0 || sViewItem[0].ItemName.indexOf("date") > 0) {
                            ...
                            if (this.resultUi.config.XFormat == "=Date(YM)") {
                                this.resultUi.config.XFormat = "YM";
                            } else if (this.resultUi.config.XFormat == "=Date(Y)") {
                                this.this.resultUi.config.XFormat = "Y";
                            } else {
