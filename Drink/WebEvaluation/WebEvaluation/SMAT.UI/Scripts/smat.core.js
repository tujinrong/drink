...
Modernizr.addTest('ie6', function () { return !!navigator.userAgent.match(/MSIE 6/i) });
Modernizr.addTest('ie7', function () { return !!navigator.userAgent.match(/MSIE 7/i) });
    ...
    smat.pagerSender.prototype = {
        ...
        close: function (result) {
            if (this.config.dynamics == true && this.config.PageId != undefined) {
                ...
                if (!pPage || !pPage.config.dataRefresh)
                {
        ...
        getFormParam: function () {
            ...
            if (this.config.parentPageId) {
                if (smat.dynamics.uiMap.contains(this.config.parentPageId.replace("page_", ""))) {
                    return smat.dynamics.uiMap.get(this.config.parentPageId.replace("page_", ""));
                }
            }
            ...
            return this.config.PageId.replace("page_", "");
        }, isHasAuth: function (auth) {
            var p = this.getPage();
            if (p && p.authSet && !p.authSet[key]) {
                return false;
            }
            ...
            if (p && p.app) {
                ...
                p.app.navigate(url, transition);
            }
        }, enable: function (enable) {
            var uis = $("#" + this.config.PageId).find('[uuid]');
            $.each(uis, function () {
                if ($(this).ui()) {
                    $(this).ui().enable(enable);
                    if (enable == false && $(this).ui() instanceof smat.Grid) {
                        $(this).ui().showForPrint();
                    }
                }
