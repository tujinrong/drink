    ...
    smat.Refer.prototype = {
        ...
        initAnimationDom: function () {
            if ($('.s-animation').length > 0) {
                ...
                this.animationUl = $('<ul unselectable="on" class="s-list s-reset" tabindex="-1" style="overflow: auto;">').appendTo(this.animationBox);
        ...
        initForInput: function () {
            ...
            var iconStr = "s-icon s-i-search";
            if (this.config.icon) iconStr = this.config.icon;

            this.pickBtn = $('<span unselectable="on" class="s-select" role="button"><span unselectable="on" class="' + iconStr + '"></span></span>').appendTo(this.picker);
            ...
            if (this.config.visible == false || this.config.visible == 'false') {
                this.visible(false);
            }
            ...
            if (this.cBool(flag) == false) {
                ...
                this.picker.addClass("s-state-disabled");
                ...
                this.picker.removeClass("s-state-disabled");
                if (smat.uiConfig.hidePickerOnDisabled) {
        ...
        , doAnimation: function (keyCode) {
            ...
            if (smat.global.referDataSourceMap.contains(this.cacheKey)) {
                ...
                var couCheck = true;
                if (self.config.checkRefer != undefined) {
                    couCheck = (this.trigger(this.config.checkRefer) == true) && couCheck;
                }
            ...
            for (var fkey = 0, lengthFkey = this.popDatas.length; fkey < lengthFkey; ++fkey) {
                ...
                if (this.referInfo.autoTemplate) {
                    var itemDomStr = this.trigger(this.referInfo.autoTemplate, viewItem);
                    var itemDom = $(itemDomStr);
                    itemDom.addClass("s-item");
                    itemDom.attr("r-key", fkey);
                    itemDom.appendTo(this.animationUl);
                } else {
            ...
            if (this.config.getParam != undefined) {
                //param = this.config.getParam();
                this.trigger(this.config.getParam, param);
            }

            smat.service.refer({
                title: smat.service.cultureText(this.referInfo.title),
                referInfo: this.referInfo,
                param: param,
                parentPageId:this.config.PageId,
        ...
        , doGetValue: function () {
            ...
            if (key.trim().length == 0) {
                ...
                if ("" != this.valueInput.val()) {
                    ...
                    this.trigger(this.config.afterSetValue,null);
                }
                this.displayInput.val("");
                this.valueInput.val("");
        ...
        , doSetValue: function (data) {
            ...
            if (data != null) {
                ...
                if (self.config.print) {
                    self.displayInput.attr("value", data[self.displayField]);
                }
                ...
                this.dataSource = null;
                if (this.config.referType == "combo") {

                    self.valueInput.val(self.displayInput.val());
                    if (self.config.print) {
                        self.displayInput.attr("value", self.displayInput.val());
                    }
        ...
        }, setReferKey: function (referKey) {
            
            if (smat.global.referInfo[referKey] != undefined) {
