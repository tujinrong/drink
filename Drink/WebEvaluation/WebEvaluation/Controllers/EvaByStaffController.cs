...
namespace WebEvaluation.Controllers
{
    ...
    public class EvaByStaffController : Controller
    {
        private EvaluationContext db = new EvaluationContext();

        /*上長評価入力-----2014-07-04-----李*/
        public ActionResult EvaByStaffIndex(string ShopCD, string PartyDateFrom, string PartyDateTo, string StaffEva, string LeaderEva, string isPostBack, string Range, string divisionCD, string StaffCD, int? page, string msg,string msgType)
        {
            ViewBag.msg = msg;
            ...
            if (isPostBack != null && isPostBack == "1")
            {
                ...
                DyPagedList<EvaBystaffIndexViewModel> lit = new DyPagedList<EvaBystaffIndexViewModel>(models, pageNumber, pageSize, totalItemCount);
        ...
        public ActionResult csv(string ShopCD, string PartyDateFrom, string PartyDateTo, string StaffEva, string LeaderEva,string Range, string divisionCD, string StaffCD)
        {
            ...
            List<EvaBystaffIndexViewModel> models = GetModels(shopCD_Change, PartyDateFrom, PartyDateTo, StaffEva, LeaderEva, Range, divisionCD, staffCD_Change,2000, 0, false, ref totalItemCount).ToList();
            ...
            String fileName = string.Format("\\CSV\\カスタマセンターの評価パーティ_{0}.csv", date);

            CsvUtils.ModlesToCsv<EvaBystaffIndexViewModel>(basePath + fileName, models);
            
            return Json(new { Path = string.Format("/CSV/カスタマセンターの評価パーティ_{0}.csv", date), ResultType = EnumResultType.Success });
        }

        //全データ取得
        private IEnumerable<EvaBystaffIndexViewModel> GetModels_old(string ShopCD, string PartyDateFrom, string PartyDateTo, string StaffEva, string LeaderEva,string Range, string divisionCD, string StaffCD)
        {
            ...
            var models = from party in Partys
                         ...
                         //where (p_report != null)
                         select new EvaBystaffIndexViewModel
                         {
        ...
        //全データ取得
        private List<EvaBystaffIndexViewModel> GetModels(string ShopCD, string PartyDateFrom, string PartyDateTo, string StaffEva, string LeaderEva, string Range, string divisionCD, string StaffCD,int pageSize, int pNumber, bool GetPageCount, ref int total)
        {
            ViewBag.ShopCD = ShopCD;
            ViewBag.PartyDateFrom = PartyDateFrom;
            ViewBag.PartyDateTo = PartyDateTo;
            ...
            List<EvaBystaffIndexViewModel> models = new List<EvaBystaffIndexViewModel>();

            if (list.DataTable.Rows.Count > 0)
            {
                foreach (DataRow dr in list.DataTable.Rows)
                {
                    EvaBystaffIndexViewModel m = new EvaBystaffIndexViewModel();
