	...
	smat.Grid.prototype = {
		...
		init : function() {
		   ...
		   //初始化grid
			...
			this.grid = $("#" + this.config.id).asmatGrid(this.getConfigForGrid()).data('asmatGrid');
		},
	         	...
	         	if(self.config.rowDblclick != undefined){
				    ...
				    $.each(items,function(n,value) { 
		          		$(this).bind('dblclick',function(e){
		          			var grid = $("#" + self.config.id).data('asmatGrid');
...
//										self.setDataSource(self.config.dataSource);
						...
						if(col.editor != undefined){
							...
							asmatDropDownList.setDataSource(divData);
		...
		reload: function(goToPage) {
			...
			//if($("#" + this.config.id +" .k-grid-pager").length > 0)
			{
				...
				if($("#" + this.config.id +" .k-grid-pager").length > 0){
					pageSize = $("#" + this.config.id+" input.grid_pageSize").data("asmatDropDownList").value();
			...
			this.destroyGrid();
			this.grid = $("#" + this.config.id).asmatGrid(this.getConfigForGrid(newDataSource)).data('asmatGrid');
		},
		...
		,delRow: function(index) {
    		...
    		if($("#" + this.config.id).data("asmatGrid")){
	    		$("#" + this.config.id).data("asmatGrid").destroy();
	    	}
	...
	smat.TreeView.prototype = {
		...
		init : function() {
		   ...
		   asmat.destroy($("#" + this.config.id));
			...
			$("#" + this.config.id).data("asmatTreeView").expand(".k-item");
		},
		...
		smat.Editor.prototype = {
			...
			init : function(config) {
			   ...
			   $(this.config.targit).attr('uuid',uuid);
				...
				//初始化控件
			   this.editor=$(this.config.targit).asmatEditor({
			    	culture: "zh-CN",
			            tools: [
