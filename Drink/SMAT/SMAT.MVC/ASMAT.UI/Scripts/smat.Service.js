...
//			}
            ...
            var onClose = function() {
            	...
            	if(afterClose !=null && afterClose != undefined){
                	 afterClose(window.data("kendoWindow").options.actionResult);
                	 if(window.data("kendoWindow").options.actionResult==undefined){
                		 if(smat.Guide!=undefined && smat.Guide.currentGuide != undefined){
                			 if($(window.data("kendoWindow").element).find(smat.Guide.currentStep.formId).length > 0){
                				 smat.Guide.moveBack();
                			 }
...
//               });
            ...
            if(fillTarget != undefined){
            	...
            	wConfig.width = "100%";
                    ...
                    window.kendoWindow(wConfig).data("kendoWindow").open();
                    
   	       			window.parent().attr('old-index',window.parent().css('z-index'));
		   	       	window.parent().bind("DOMSubtreeModified", function(e) { 
   	       			if(smat.Guide!=undefined && smat.Guide.currentGuide != undefined && window.data("kendoWindow")){
      	       			 if($(window.data("kendoWindow").element).find(smat.Guide.currentStep.formId).length > 0){
		...
		doCloseSubForm: function (result){			
			...
			var window = $("#"+result.formId).parent().data("kendoWindow");
			
			//平板的时候。。。。。
			if(window == undefined){
				window = $("#"+result.formId).parent().parent().data("kendoWindow");
			}
...
//			if($("#" + targer+" input.grid_pageSize").data("kendoDropDownList")){
//				//alert($("#" + targer+" input.grid_pageSize").data("kendoDropDownList"));
//				$("#" + targer+" input.grid_pageSize").data("kendoDropDownList").destroy();
//			}
					...
					smat.service.doJsonURLNormal(
						{
							...
							params:{
								condition:{
									...
									onPage:true,
									pageSize:sizeBox.data("kendoDropDownList").value()
								}
					...
					if(data[fkey]!=undefined){
						if(info.type == "date"){
							if(data[fkey] != null){
								...
								data[fkey] = kendo.toString(kendo.parseDate(data[fkey].toString(),"yyyyMMdd"),"yyyy/MM/dd");
							}
							...
							if(data[fkey] != null){
								...
								data[fkey] = kendo.toString(kendo.parseDate(data[fkey].toString(),"yyyyMMddHHmm"),"yyyy/MM/dd HH:mm");
							}
				            ...
				            if($(this).attr("name")!=undefined && $(this).attr("name").length > 0){
				            	if(backup[$(this).attr("name")] != undefined){
				            		...
				            		if($(this).data("kendoDropDownList")){
										$(this).data("kendoDropDownList").value(backup[$(this).attr("name")]);
									}else if($(this).data("kendoNumericTextBox")){
										$(this).data("kendoNumericTextBox").value(backup[$(this).attr("name")]);
									}else if($(this).data("kendoEditor")){
										$(this).data("kendoEditor").value(backup[$(this).attr("name")]);
									}else if($(this).data("kendoDatePicker")){
										$(this).val(kendo.toString(kendo.parseDate(backup[$(this).attr("name")],"yyyyMMdd"),"yyyy/MM/dd"));
									}
									else if($(this).data("kendoDateTimePicker")){
										$(this).val(kendo.toString(kendo.parseDate(backup[$(this).attr("name")],"yyyyMMddHHmm"),"yyyy/MM/dd HH:mm"));
									}
									else if($(this).data("kendoTimePicker")){
										var timeVal = backup[$(this).attr("name")].replace(/:/g,'').padLeft(4,'0');
										$(this).data('kendoTimePicker').value(timeVal.substring(0,2)+":"+timeVal.substring(2,4));
				 ...
				 $.each(ctls,function(n,value) { 
			            if($(this).attr("name")!=undefined && $(this).attr("name").length > 0){
			            	if($(this).data("kendoDropDownList")){
								...
								$(this).data("kendoNumericTextBox").enable(enable);
							}
							else{
								if(enable == false){
		...
		doCommonInit: function(targit){
			...
			var btns = $(targit).find(".s-button");
			 ...
			 $.each(calendarInputs,function(n,value) { 
				...
				$(this).val(kendo.toString(kendo.parseDate($(this).val(),"yyyyMMdd"),"yyyy/MM/dd"));
			 ...
			 $.each(dataTimeInputs,function(n,value) { 
				$(this).val(kendo.toString(kendo.parseDate($(this).val(),"yyyyMMddHHmm"),"yyyy/MM/dd HH:mm"));
			 ...
			 $.each(timepickerInputs,function(n,value) { 
				...
				//$(this).data('kendoTimePicker').value($(this).val());
           });
...
//						}
			...
			//数字
			 $(targit).find(".s-number").kendoNumericTextBox({
				 format: "n0"
			 });
			 
			//拾色器
			 $(targit).find(".s-color").kendoColorPicker({
		            value: "#ffffff",
		            buttons: false
		        });
				...
				smat.service.doOpenSubForm(
						{
							...
							afterClose:function(result){
								if(result != undefined &&
										   ...
										   result.selectedRow != null){
											var item = result.selectedRow;
											var editor = $("#"+editerTargit).data("kendoEditor");
											//editor.exec("Signature", { text: '<img src="/resident/'+item.RESOURCE_URL+'" />' });
											editor.paste('<img src="/resident/'+item.RESOURCE_URL+'" />');
											setTimeout(function(){editor.focus();},1000);
										}
			...
			for (var key in result){
				...
				if(control.roleKey != null && control.roleKey != undefined){
					...
					if(control.type == "Grid"){
						$("#" + control.roleKey).kendoGrid(control);
					}
					else if(control.type == "Chart"){
						$("#" + control.roleKey).kendoChart(control);
					}
					else if(control.type == "DropDownList"){
						
						$("#" + control.roleKey).kendoDropDownList(control);
					}
				}
			}
		},
...
//						}
		...
		endLoding :function (target){
			...
			link.attr('href',SMAT.Global.basePath +'/ui/styles/kendo.'+style+'.min.css');
			link_mobile.attr('href',SMAT.Global.basePath +'/ui/styles/kendo.'+style+'.mobile.min.css');
		...
		showTip: function (config){
			if($("#SYS_tipTemp").length == 0){
				...
				smat.Global.CommonTip =  $("#SYS_tipTemp").kendoTooltip({
                    autoHide: true,
                    showOn: "click",
                    position: "top",
                    content: "AMO",
		...
		notice: function (config){
			if($("#SYS_notificationTemp").length == 0){
				...
				smat.Global.CommonNotice = $("#SYS_notificationTemp").kendoNotification({
	                    position: {
	                    pinned: true,
	                    top: 0,
	                    left: 600
...
//			}
		...
		openNewTabForm:function(formKey){
			...
			var tabStrip = $("#pageTab").data("kendoTabStrip");
			...
			if(tab.length > 0){
				...
				$("button[key="+id+"]").bind('click',function(e){
	                ...
	                kendo.destroy($("#"+tabToDel.attr('aria-controls')));
