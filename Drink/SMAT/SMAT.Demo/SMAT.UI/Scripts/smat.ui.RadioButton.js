(function () {
    ...
    smat.RadioButton.prototype = {
        ...
        initForInput: function () {
            ...
            if (this.config.dataSource != undefined)
            {
                ...
                for (var i = 0; i < this.dataSource.length; i++) {
                    ...
                    $(this.config.target).before($('<label class="control-label radio m-l-md  i-checks" style="text-align:left;"><input type="radio" data-item-key="' + i + '" class="chs-item" name="' + this.name + '_rdo" value="' + this.dataSource[i]["value"] + '" ' + checked + '><i></i>' + this.dataSource[i]["text"] + '</label>'));
                }
            }
        ...
        text: function () {
            ...
            var key = $("[name='" + this.name + "']:checked").attr("data-item-key");
            return this.dataSource[key]["text"];
        }
    };
    // extend Node
