...
(function () {
    ...
    smat.dynamics.FormList.prototype = {
        ...
        init: function () {
            ...
            this.rowtoolBar = $('<div class="row" style="margin: 3px 0 0 0;"><button id="designer_form_list_search_btn" class="btn-dark s-button" style="margin-left:5px;"><i class="icon-reload"></i>　' + smat.service.optionSet("DyOptionText.Refresh") + '</button></div>').appendTo(this.toolBarContent);
            ...
            this.saveBtn = $('<button  class="btn-dark s-button" style="margin-left:5px;"><i class="fa fa-save"></i>　' + smat.service.optionSet("DyOptionText.Save") + '</button>').appendTo(this.rowtoolBar);
            this.designBtn = $('<button  class="btn-dark s-button" style="margin-left:5px;"><i class="fa fa-edit"></i>　' + smat.service.optionSet("DyOptionText.Design") + '</button>').appendTo(this.rowtoolBar);
            this.savForeBtn = $('<button  class="btn-dark s-button" style="margin-left:5px;"><i class="fa fa-save"></i>　' + smat.service.optionSet("DyOptionText.SaveFor") + '</button>').appendTo(this.rowtoolBar);
            this.delBtn = $('<button class="btn-dark s-button" style="margin-left:5px;"><i class="fa fa-trash-o"></i>　' + smat.service.optionSet("DyOptionText.Delete") + '</button>').appendTo(this.rowtoolBar);
            this.closeBtn = $('<button class="btn-dark s-button" style="margin-left:5px;"><i class="fa fa-times"></i>　' + smat.service.optionSet("DyOptionText.Close") + '</button>').appendTo(this.rowtoolBar);
        ...
        getNewFormName: function (handle) {
            ...
            $('<div class="row" style="margin:8px 0;"><div class=" form-group" ><label class="control-label input-s-sm text-right" style="margin-right:5px;">' + smat.service.optionSet("DyOptionText.Type") + '</label><input id="_NEW_Form_Entity" class="input-s" ></div></div>').appendTo(ebox);
            $('<div class="row" style="margin:8px 0;"><div class=" form-group" ><label class="control-label input-s-sm text-right" style="margin-right:5px;">' + smat.service.optionSet("DyOptionText.Name") + '</label><input id="_FilterName_NEW" class="s-textbox input-s" ><button id="_pick_newName" class="btn-info " style="margin-left:10px;">ok</button></div></div>').appendTo(ebox);
            var newNameInput = ebox.find("#_FilterName_NEW");

            var newEntityInput = ebox.find("#_NEW_Form_Entity");

            newNameInput.val(this.rowCondition.find(".sm-state-active").text().trim() + smat.service.optionSet("DyOptionText.New"));
            ...
            smat.service.loadJosnData({
                ...
                success: function (result) {
                    ...
                    newEntityInput.ui().uiControl.setDataSource(result);
            ...
            this.delBtn.bind("click", function (e) {
                if (self.pageItem != null) {
                    var confirm_config = {
                        msg: smat.service.optionSet("DyOptionMsg.Delete",self.pageItem.FormName),
                        callback: function () {
                            smat.service.loadJosnData({
                                ...
                                success: function (result) {
                                    smat.service.notice({ msg: smat.service.optionSet("DyOptionMsg.DeleteSuccess") });
                                    self.refreshTree();
                                    self.clearCurrent();
                                }
                            });
