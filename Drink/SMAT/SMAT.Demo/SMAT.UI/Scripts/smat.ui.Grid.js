(function () {
    ...
    smat.Grid.prototype = {
        ...
        getConfigForGrid: function (newDataSource) {
                ...
                if (self.config.select != undefined) {
                    ...
                    $.each(items, function (n, value) {
                        $(this).bind('click', function (e) {
                            ...
                            if ($(self.config.target).find('tbody').children('tr.s-state-selected').index() != $(this).index()) {
            ...
            if (this.config.category != undefined && this.config.category != ""
                ...
                for (var sKey in tempData) {
                    ...
                    var kIndex = 0;
                    for (var k in this.config.series) {
                        sItem[this.config.series[k].seriesField] = sKeys[kIndex];
                        kIndex++;
                    }
                    ...
                    for (var xKey in tempData[sKey]) {
                        ...
                        sItem["seriesField"+xKey] = tempData[sKey][xKey];
                ...
                for (var xKey in xValues) {
                    ...
                    var xCol = {
                        ...
                        field: "seriesField" + xValues[xKey],
                        attributes: {
                            "class": "text-right"
                        }
                    }
            ...
            if (this.config.category_format != "" && this.config.category_format != undefined) {
                ...
                if (this.config.category_format.indexOf("codeKind:") >= 0) {
                    var kind = this.config.category_format.replace("codeKind:", "");
                    return smat.service.optionSet({ codeKind: kind, cd: val });
                }
                ...
                for (var fkey in this.formatInfo) {
                    ...
                    if (data[fkey] != undefined) {
                        if (info.type == "date") {
                            ...
                            if (data[fkey] != null && data[fkey].trim() != "") {
                                
                                data[fkey] = asmat.format(info.format, Number(data[fkey]));
                            }
                        } else if (info.type == "codeKind") {
                            //if (smat.global.codeMstMap[info.codeKind] != undefined) {
                            //    if (smat.global.codeMstMap[info.codeKind][data[fkey]] != undefined) {
                            //        //data[fkey + "_CD"] = data[fkey];
                            //        data[fkey + "_Formated"] = smat.global.codeMstMap[info.codeKind][data[fkey]][smat.uiConfig.CodeMst.nameField];
                            //    } else {
                            //        data[fkey + "_Formated"] = "";
                            //    }
                            //}
                    ...
                    for (colKey in self.editInfo) {
                        ...
                        { 
                            if (dataItem[colInfo.field] == null || dataItem[colInfo.field] == undefined || String(dataItem[colInfo.field]) == "") {
                                ...
                                smat.service.addErrorInfo(self.uuid + colInfo.field, node, "【" + colInfo.title + "】を入力してくだい。");
                            }
                        }
