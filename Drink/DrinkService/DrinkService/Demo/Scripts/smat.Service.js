...
(function() {
	...
	smat.service = {
		...
		doOpenSubForm: function (config){
			...
			$('body').append("<div id='MainSubForm_" + timeId + "' style='display: none;'><div class='loadingBox' style='margin-top:10%; text-align: center;'><img class='s-image' src='Styles/Material/loading-image.gif'></div></div>");
            ...
            var onOpen = function() {
            	...
            	if(fillTarget != undefined){
    				...
    				$('#'+closeBtn +'_close').bind('click',function(e){
                     ...
                     window.parent().find('.s-window-titlebar').hide();
                     window.parent().find('.s-resize-handle').hide();
...
//			}
			content.find(".s-pager-wrap").remove();
			
			//外框
			var box = $('<div class="s-pager-wrap s-grid-pager s-widget" data-role="pager" ></div>').appendTo(content);
			
			//到第一页
			var goFirst = $('<a href="#" title="跳转到第一页" class="s-link s-pager-nav s-pager-first" data-page="1" tabindex="-1"><span class="s-icon s-i-sees-w">跳转到第一页</span></a>').appendTo(box);
			if(pageNumber <= 1){
				goFirst.addClass("s-state-disabled");
			}
			
			//向前一页
			var goPre = $('<a href="#" title="跳转到上一页" class="s-link s-pager-nav" data-page="'+(pageNumber-1)+'" tabindex="-1"><span class="s-icon s-i-arrow-w">跳转到上一页</span></a>').appendTo(box);
			if(pageNumber <= 1){
				goPre.addClass("s-state-disabled");
			}
			
			//页数列表
			var pageUl = $('<ul class="s-pager-numbers s-reset"></ul>').appendTo(box);
		
			//前面更多 。。。
			if(p_begin > 1){
				$('<li><a tabindex="-1" href="#" class="s-link" data-page="'+(p_begin-1)+'" title="More pages">...</a></li>').appendTo(pageUl);
			}
			
			for(var index = p_begin; index < (p_begin+p_show);index ++){
				if(index <= p_total){
					if(index == pageNumber){
						$('<li><span class="s-state-selected" current="1">'+index+'</span></li>').appendTo(pageUl);
					}else{
						$('<li><a tabindex="-1" href="#" class="s-link" data-page="'+index+'">'+index+'</a></li>').appendTo(pageUl);
					}
				}
			}
			...
			if((p_begin+p_show) < p_total){
				$('<li><a tabindex="-1" href="#" class="s-link" data-page="'+(p_begin+p_show)+'" title="More pages">...</a></li>').appendTo(pageUl);
			}
			
			//向后一页
			var goNexte = $('<a href="#" title="跳转到下一页" class="s-link s-pager-nav " data-page="'+(pageNumber+1)+'" tabindex="-1"><span class="s-icon s-i-arrow-e">跳转到下一页</span></a>').appendTo(box);
			if(pageNumber == p_total|| p_total ==0){
				goNexte.addClass("s-state-disabled");
			}

			//到最后一页
			var goLast = $('<a href="#" title="跳转到最后一页" class="s-link s-pager-nav s-pager-last " data-page="'+p_total+'" tabindex="-1"><span class="s-icon s-i-sees-e">跳转到最后一页</span></a>').appendTo(box);
			if(pageNumber == p_total || p_total ==0){
				goLast.addClass("s-state-disabled");
			}
			...
			sizeBox.asmatDropDownList(
					{ 
						...
						value: pageSize});
			//分页信息
			var pageInfo = $('<span class="s-pager-info s-label"></span>').appendTo(box);
			if(pageNumber == p_total || p_total ==0){
			...
			$("#" + targer + " a[data-page]").bind("click",function(e){
				if($(this).hasClass("s-state-disabled")== false){
...
//			       	    });
//					//已选择的按钮直接不响应事件
//						if($(e.sender.element).hasClass("s-state-selected")){
//							return;
//						}
		...
		doSetSubMenuInfo: function(menuKey,fctKey){
			...
			if(selectMenu != null){
				smat.Global.module = menuKey;
	       	    ...
	       	    btn.addClass("s-state-selected"); 
	       	    
	       	    
	       	    var li = $("#menu li[menu='"+menuKey+"']");
	       	    li.addClass("s-state-disabled");
	       	    li.addClass("s-state-selected"); 
	       	    li.removeClass("s-state-hover");
		...
		showErrorInfo: function(errorInfos){
			...
			for(var key in errorInfos){
				...
				if(node.length > 0){
					...
					if(node.parent().parent().hasClass("s-datepicker")){
...
//						}
		...
		beginLoding :function (target){
			...
			$(target).append("<div class='loadingBox' style='margin-top:10%; text-align: center;'><img class='s-image' src='"+SMAT.Global.basePath+"/ui/styles/BlueOpal/loading-image.gif'></div>");
		}
		,
...
//			$('body').append('<div id="MainLodingForm_overlay" class="s-overlay" style="z-index: 10002; opacity: 0.5;background-color: transparent;"></div>');
//			
//			$('body').append("<div id='MainLodingForm' style=''><div class='loadingBox' style='margin-top:10%; text-align: center;'><img class='s-image' src='"+SMAT.Global.basePath+"/ui/styles/BlueOpal/loading-image.gif'></div></div>");
//			
//			var width = "100px";
//			
//			var modal = false;
...
//			}
		...
		openNewTabForm:function(formKey){
			...
			if(tab.length > 0){
				...
				var timeId = parseInt(new Date().valueOf() / 1000);
				tabStrip.append({
	                                    text: "<span key='"+id+"'>"+text+"</span><button type='button' key='"+id+"' class='s-tab-button s-button s-button-icontext' ><img alt='icon' class='s-image' style='margin: 0;' src='"+SMAT.Global.basePath+"/images/style1/16x16/broken.png'></button>",
	                                    encoded: false,
	                                    imageUrl:imageUrl,
