...
(function () {
    ...
    smat.service = {
        ...
        openForm: function (config) {
            ...
            smatWindow.parent().attr('old-index', smatWindow.parent().css('z-index'));
        ...
        closeForm: function (config) {
            ...
            var windowBox = $("#" + contentId).closest('.s-window-content');
                    ...
                    if (value.length == 0) {
                        ...
                        if (form_group.length > 0) {
                            if (form_group.find('.control-label').length > 0) {
                                cpation = "【" + form_group.find('.control-label').text() + "】:";
                            }
                        }
        ...
        getJsonData: function (data, key) {
            smat.service.getJsonDataWithEmptyObject(data, key, "");
        },
        ...
        dialog: function (config) {
            var uid = smat.service.uuid();
            var box = $('<section id="' + uid + '_confirm" class="panel panel-default " style="margin: 0;padding: 10px;height: 130px;"></section>');
            ...
            var btnRow = $('<div class="row" style="margin:8px 0;padding-top: 30px;padding-bottom: 5px;"></div>').appendTo(box);
            ...
            for (var i in btns) {
                ...
                var btnControl = $('<button id="_pick_cancel" index="' + i + '" class="' + cls + '" style="margin-left:10px;">' + btn.lbl + '</button>');
                btnControl.appendTo(btnGroup);

                btnControl.smatButton({
                    click: function (e) {
                        var index = this.element.attr("index")
        ...
        notice: function (config) {
            ...
            if (config.target != undefined) {
                ...
                h = box.height();
                var w = $('.s-notification').closest('.s-animation-container:visible').width();
                var ww = $(window).width();
                //if ((left + w) > ww) left = ww - w;
                if (Modernizr.ios || Modernizr.android) h = h + 20;
                $('.s-notification').closest('.s-animation-container:visible').css("top", (top + h) + "px");
                //$('.s-animation-container:visible').css("left", left + "px");
            }


            $('.s-notification').closest('.s-animation-container').css("z-index", "999999");
        },
