    ...
    smat.service = {
        ...
        loadJosnData: function (config) {
            ...
            if (smat.global.localServer == true) {
                ...
                window.LoadJsonData(url, JSON.stringify(params).replace(new RegExp("\"null\"", "gm"), "\"\""), uid);
                smat.global.callCSharpMap.set(uid, { success: success });
                smat.service.setLastSessionTime();
                // wait..
                if (async == false) {
                    //while (window.CheckAsync(uid) == "false") {
                    //    ///
                    //}
                }
        ...
        localCallBack: function (uid, resultStr) {
            ...
            if (info.success != null && info.success != undefined) {
                info.success(smat.service.strToJson(resultStr));
            }
        ...
        beginLoding: function (target) {
            ...
            $(target).append('<div class="loadingBox"><img src="/SMAT.UI/Styles/Material/loading-image.gif" /></div>');
        }
		,
        endLoding: function (target) {
            $(target).find(".loadingBox").remove();;
        },
        canvasDraw: function (config) {
            var title = config.title;
            var width = config.width;
            var height = config.height;
            var data = config.data;
            var afterDraw = config.afterDraw;

            var pageBox = $('<div class="panel-body" id="referShopListForm" style="padding:5px 20px">');
            var box = $('<div class="form-horizontal">').appendTo(pageBox);
            var row1 = $('<div class="row">').appendTo(box);
            $('<div class="line line-dashed b-b line-lg pull-in "></div>').appendTo(box);
            var row2 = $('<div class="row">').appendTo(box);

            var controlBox = $('<div class="form-group">').appendTo(row1);

            $('<label class="input-s-sm control-label">幅</label>')//.appendTo(controlBox);
            var widthDrop = $('<input id="widthDrop" type="text" class="form-control input-s-sm ">')//.appendTo(controlBox);
            widthDrop.asmatDropDownList({
                dataTextField: "text",
            ...
            var widthControl = widthDrop.data("asmatDropDownList");

            $('<label class="input-s-sm control-label">カラー</label>')//.appendTo(controlBox);
            var colorDrop = $('<inputid="colorDrop" type="text" class="form-control input-s-sm ">')//.appendTo(controlBox);
            colorDrop.asmatDropDownList({
                dataTextField: "text",
                dataValueField: "value",
                dataSource: [
            ...
            var colorControl = colorDrop.data("asmatDropDownList");
            var clearBtn = $('<button type="button" class="btn-primary s-button">クリア</button>').appendTo(controlBox);
            var saveBtn = $('<button type="button" class="btn-primary s-button" style="margin-left: 5px;">保存</button>').appendTo(controlBox);


            var canvasBox = $('<div class="form-group">').appendTo(row2);

            var canvaWidth = 952; //$(window).width() * 0.95 - 20;
            var canvaHeight = 400; //$(window).height() * 0.92 - 70;

            var canvasId = smat.service.uuid();
            var canvasDom = $('<canvas id="' + canvasId + '" width="' + canvaWidth + '" height="' + canvaHeight + '" style="border:2px solid #6699cc;"></canvas>').appendTo(canvasBox);
            ...
            var doDraw = function (x, y, isDown) {
                if (isDown) {
                    ctx.beginPath();
                    ctx.strokeStyle = "black";//colorControl.value();
                    ctx.lineWidth = "8";//widthControl.value();
                    ctx.lineJoin = "round";
            ...
            var drawStart = function (e) {
                ...
                mousePressed = true;
                doDraw(e.pageX - $(this).offset().left, e.pageY - $(this).offset().top, false);
            };

            var drawMove = function (e) {
                event.preventDefault();
                if (mousePressed) {
                    doDraw(e.pageX - $(this).offset().left, e.pageY - $(this).offset().top, true);
                }
            };
            ...
            $(canvas).bind("mousedown", drawStart);
            $(canvas).bind("mousemove", drawMove);
            $(canvas).bind("mouseup mouseleave", drawEnd);
        ...
        openForm: function (config) {
            ...
            if (config.contentDom == undefined) {
                if (Modernizr.ie9 == true && config.url != "/Dynamics/FormPage") {
                    debugger
                    iframe = true;
                }
            }
            ...
            $.each(inputs, function (n, value) {
                ...
                if (name != '' && $(this).hasClass("s-required")) {
                    if (value.length == 0) {
                        ...
                        smat.service.addErrorInfo(name, $(this), cpation + "を入力してくだい。");
                    }
                }
            });
        ...
        dialog: function(config){
            ...
            $('<div class="row" style="margin:8px 0;"><div class=" form-group" style=" text-align: center;"><label class="control-label text-left" style="max-height: 460px;overflow-y: auto;">' + config.content + '</label></div></div>').appendTo(box);
        ...
        getUserConfig: function (config) {
            ...
            for (var k in items) {
                ...
                var input = $('<input name="' + item.key + '" style="float:left; ' + style + '"/>').appendTo(inputBox);
            ...
            var btnOK = $('<button class="btn-info" style="margin-left:10px;">設定</button>');
            ...
            var btnCanel = $('<button class="btn-info" style="margin-left:10px;">取消</button>');
		...
		, loadHash: function (e) {
		    ...
		    smat.service.closeLoding();
		    if (XMLHttpRequest.responseText.indexOf('asmat-plase-go-to-login-form') > 0) {
		        window.location.href = "/Main/Login";
		    } else {
		        ...
		        if (XMLHttpRequest.status == 0) {
		            ...
		            if (XMLHttpRequest.responseText.indexOf('JSON JavaScriptSerializer') >= 0) {
		                if (window.location.href.indexOf("BusinessAnalysis") > 0) {
		                    smat.service.notice({ msg: "リクエストデータは多すぎ。項目を減らすか、条件を絞つても一度実行してください。", type: "error" });
		                } else {
		                    smat.service.notice({ msg: "システムがビジー状態です。しばらくしてから再度処理して下さい。", type: "error" });
		                }
		            } else if (XMLHttpRequest.responseText.indexOf('The wait operation timed out') >= 0) {
		                if (window.location.href.indexOf("BusinessAnalysis") > 0) {
		                    smat.service.notice({ msg: "リクエストは複雑すぎ為、タイムアウトしました。項目を減らすか、条件を絞つても一度実行してください。", type: "error" });
		                } else {
		                    smat.service.notice({ msg: "システムがビジー状態です。しばらくしてから再度処理して下さい。", type: "error" });
		                }
		            } else if (XMLHttpRequest.responseText.indexOf('timeout') >= 0) {
		                //smat.service.notice({ msg: "リクエストは複雑すぎ為、タイムアウトしました。項目を減らすか、条件を絞つても一度実行してください。", type: "error" });
		                if (window.location.href.indexOf("BusinessAnalysis") > 0) {
		                    smat.service.notice({ msg: "タイムアウトしました。項目を減らすか、条件を絞つても一度実行してください。", type: "error" });
		                } else {
		                    smat.service.notice({ msg: "システムがビジー状態です。しばらくしてから再度処理して下さい。", type: "error" });
		                }
		...
		}, lockUIField: function (id) {
		    var uiFields = $("#" + id).find("[uuid]");
		    $(uiFields).each(function () {
		        if ($(this).ui() && $(this).ui() instanceof smat.Button == false)
        ...
        $(this).each(function () {
            ...
            if (isNaN(tempDate.getDate()) || value.replace(/\//g, "") != tempDate.format("yyyyMMdd")) {
                if (isShowMsg) {
                    //alert("日期格式不正确！");
                    //$(this).val("")
                    smat.service.notice({ msg: "日付は正しくありません。", type: "error", target: $(this) });
