        ...
        init: function () {
            ...
            if (this.config.dataHandler != undefined) {
                ...
                if (handler instanceof smat.Grid) {
                    ...
                    if (dataSource.pageCategories) {
                        handler.config.pageCategories = dataSource.pageCategories;
                        handler.config.crossYItems = dataSource.pageCrossYItems;
                    ...
                    handler.startIndex = (this.pageNumber - 1) * this.dataSource.pageSize;
                    if (dataSource.pageSeries) {
                        handler.setSeriesDataSource(dataSource.pageSeries, dataSource.pageCategories);
                    } else {
                        handler.setDataSource(dataSource.pageData);
                    }
                }
            ...
            if (dataSource.pageSeries) {
                if (this.config.showNoDataMsg && (this.dataSource.pageSeries == null || this.dataSource.pageSeries.length == 0)) {
                    smat.service.notice({ msg: smat.service.optionSet("SysMsg.NoData"), type: "info" });
                }
            } else {
                if (this.config.showNoDataMsg && (this.dataSource.pageData == null || this.dataSource.pageData.length == 0)) {
                    smat.service.notice({ msg: smat.service.optionSet("SysMsg.NoData"), type: "info" });
                }
            }
