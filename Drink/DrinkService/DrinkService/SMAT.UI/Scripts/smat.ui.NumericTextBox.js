(function () {
    ...
    smat.NumericTextBox.prototype = {
        ...
        init: function () {
            ...
            if (this.config.select == false || this.config.select == 'false') {
                this.initSelect();
            }
            ...
            if (Modernizr.ios || Modernizr.android) {
                this.uiControl.element.attr("readonly", "readonly");
            };
            ...
            this.animationHeader = $('<div class="s-header" style="height:45px;text-align: left;"><input class="s-input s-textbox" readonly="true" style="margin-top: 5px;margin-left: 15px;width: 190px;float: left;text-align: right;font-size: 28px;padding: 0 5px;line-height: 36px;height: 36px;" value="0" /></div>').appendTo(this.animationBox);
            ...
            if (this.animation.is(":visible") == false) {
                ...
                var wW = $(window).width();
                var bW = this.animation.width();

                if ((this.left + bW) > wW) {
                    this.animation.css("left", (wW-bW-5) + "px");
                } else {
            ...
            this.animation.fadeOut(this.speed, function () {
                //self.uiControl.enable(true);
                $(".s-overlay").remove();
            });
        }, pickOk: function () {
            var self = this;
            ...
            if (this.cBool(visibleFlag) == false) {
                $(this.config.target).closest(".s-numerictextbox").hide();
                ...
                $(this.config.target).closest(".s-numerictextbox").show();
                if (this.config.label != undefined) {
                    this.labelDom.show();
                }
                if (this.config.inputBox != undefined) {
