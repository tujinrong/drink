...
{
    public class xxxHoOrderClientLogic :LogicBase
    {
        public xxxHoOrderClientLogic(EntityRequest enreq)
            : base(enreq)
        {
            _enreq = enreq;
        }
        ...
        private IEnumerable<DeliveryRouteListViewModel> GetModels(string shopCD, string staffCD, string route, string hoDate)
        {
            ...
            //{
            //    DateTime hoDate = Convert.ToDateTime(date);
            //    models = models.Where(m =>m.HoDate.CompareTo(hoDate) == 0);
            //}
        ...
        public List<DeliveryRouteClientViewModel> GetClientList(string shopCD, string staffCD, string routeCD, string hoDate, out  List<DeliveryRouteClientViewModel> dataSaved)
        {
            ...
            //List<T_HoOrderTanto> oldTantos = db.HoOrders.Where(e => e.ShopCD == shopCD && e.HoDate == dt).ToList();

            //var HoDayClients = HoOrderClients.Where(e => e.HoDate == dt).ToList();

            //var HoBeforeDayClients = HoOrderClients.Where(e => e.HoDate < dt).ToList();
...
";
        ...
        public List<HoClientItemViewModel> GetHoClientItems(string ShopCD, string ClientCD, DateTime HoDate, string FirstFlag, string TantoCD,string Route)
        {
            ...
            { 
                ...
                //                  where ppi.ShopCD == orderClient.ShopCD
                //                     && ppi.ClientCD == orderClient.ClientCD
                //                     && ppi.Seq == (orderClient.Seq - 2)
                //                  select ppi;

                //var preItems = from pi in db.HoClientItems
                //               where pi.ShopCD == orderClient.ShopCD
                //                 && pi.ClientCD == orderClient.ClientCD
                //                 && pi.Seq == (orderClient.Seq - 1)
                //               select pi;
                ...
                var clientItems = from i in nowList
                                  ...
                                  //join preItem in preList on i.ItemCD equals preItem.ItemCD into g_preItem
                                  //from pre_item in g_preItem.DefaultIfEmpty(new T_HoClientItem())

                                  //join prePreItem in prePreList on i.ItemCD equals prePreItem.ItemCD into g_prePreItem
                                  //from pre_pre_item in g_prePreItem.DefaultIfEmpty(new T_HoClientItem())

                                  //orderby i.ItemAddFlag,i.ShelfNo, i.ShelfSubNo
